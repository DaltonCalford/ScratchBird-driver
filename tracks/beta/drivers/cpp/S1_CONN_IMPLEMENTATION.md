...
1. Updated connection transport resolution in `src/network_client.cpp`:
   - `transport_mode=local_ipc` + `ipc_method=pipe` no longer returns `NOT_SUPPORTED`.
   - Pipe mode now uses a local loopback fallback path so connection/auth protocol flow can proceed in lane tests.
...
3. Expanded connection/auth/protocol coverage in `tests/test_driver_connectivity.cpp`:
   ...
   - Added local IPC pipe fallback connectivity test.
...
1. Native OS named-pipe transport is still not implemented; `ipc_method=pipe` currently uses local loopback fallback.
2. Embedded transport is still routed through local IPC rather than a true in-process engine path.
3. Lane tests still lack deterministic manager-proxy end-to-end handshake/auth coverage (success and failure matrices).
