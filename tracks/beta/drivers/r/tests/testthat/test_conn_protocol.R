...
test_that("sb_connect validates DSN/auth options before opening socket", {
  ...
  expect_error(
    sb_connect("scratchbird://user:pass@localhost:3092/mydb?binary_transfer=false"),
    "binary_transfer=false is not supported"
  )
  expect_error(
    sb_connect("scratchbird://user:pass@localhost:3092/mydb?compression=zstd"),
    "compression=zstd is not supported"
  )
  expect_false(socket_opened)
})
