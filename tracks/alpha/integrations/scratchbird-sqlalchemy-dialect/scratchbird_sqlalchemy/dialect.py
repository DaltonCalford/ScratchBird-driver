...
class ScratchBirdDialect(DefaultDialect):
    ...
    def create_connect_args(self, url):
        ...
        for raw_key, raw_value in (url.query or {}).items():
            ...
            key_lower = key.lower()
            if key_lower in {"connecttimeout", "connect_timeout"}:
                connect_args["connect_timeout"] = value
                continue
            if key_lower in {"sockettimeout", "socket_timeout"}:
                connect_args["socket_timeout"] = value
                continue
            if key_lower in {"binarytransfer", "binary_transfer"}:
                if _is_falsey(value):
                    raise ValueError("binary_transfer=false is not supported")
                connect_args["binary_transfer"] = True if _is_truthy(value) else value
                continue
            if key_lower == "sslmode" and str(value).strip().lower() == "disable":
                raise ValueError("sslmode=disable is not supported")
            if key_lower == "compression" and str(value).strip().lower() == "zstd":
