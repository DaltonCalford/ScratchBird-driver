...
| META | JDBCBL-META | Partial | `src/metadata.rs:12`, `src/metadata.rs:67`, `src/metadata.rs:96`, `src/metadata.rs:115`, `src/metadata.rs:169`, `tests/metadata_test.rs:16`, `tests/metadata_test.rs:50`, `tests/metadata_test.rs:75`, `tests/metadata_test.rs:93` (metadata helper queries now include metadata-row shaping for recursive schema navigation with parent expansion option and per-parent uniqueness; status remains partial because this lane still does not expose a first-class executable metadata API surface across full JDBC metadata families.) |
