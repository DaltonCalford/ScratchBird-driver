...
- `src/scratchbird_native.mojo:55` (native-bootstrap `user|username|pguser` / `password|passwd|pgpassword` query overrides layered over DSN userinfo parsing)
- `src/scratchbird_native.mojo:71` (native-bootstrap `host|hostname|servername|pghost`, `port|portNumber|pgport`, and `database|dbname|databaseName|pgdatabase` query overrides layered over DSN endpoint/path parsing, including bracketed IPv6 hosts)
- `src/scratchbird_native.mojo:93` (native-bootstrap session property parsing: `role`, `application_name|applicationname`, `autocommit|auto_commit`, `readonly|read_only`, `current_schema|search_path|searchpath`, `default_row_fetch_size|fetch_size|fetchsize`)
- `src/scratchbird_native.mojo:190` (native-bootstrap connection identity formatting includes endpoint context: `user@host:port/database`)
- `src/scratchbird_native.mojo:119` (native-bootstrap front-door mode normalization + source-precedence across `front_door_mode|frontdoormode` / `connection_mode` / `ingress_mode`, including `managed` normalization)
- `src/scratchbird_native.mojo:117` (native-bootstrap protocol alias parsing: `protocol|parser|dialect`)
- `src/scratchbird_native.mojo:132` (native-bootstrap timeout alias parsing: `connect_timeout|connecttimeout`, `socket_timeout|sockettimeout`, `login_timeout|logintimeout`, `acquire_timeout|acquiretimeout` with fallback `pooling_acquire_timeout|poolingacquiretimeout`)
- `src/scratchbird_native.mojo:136` (native-bootstrap lifecycle DSN knob parsing: `cb_*` / `keepalive_*` / `leak_*` / `pipeline_*`)
- `src/scratchbird_native.mojo:1154` (native-bootstrap connect guards: protocol/TLS/binary/compression/mode/user+db + endpoint/timeout/session guardrails (including `port` upper bound, non-negative `acquire_timeout`, and non-negative `default_row_fetch_size`) + deterministic auth-fail parity)
- `src/scratchbird_native.mojo:1176` (native-bootstrap `connect` entrypoint)
- `src/scratchbird_native.mojo:338` (native-bootstrap `ping` surface)
...
- `tests/scratchbird_surface.mojo:75` (current-syntax `scratchbird` facade connect/ping/query smoke + DSN credential/host/port/database alias parse assertions (including JDBC/PG aliases), session property alias parsing, protocol alias parsing, and mode normalization/precedence checks)
- `tests/scratchbird_surface.mojo:89` (facade smoke asserts bracketed IPv6 host/port parsing)
- `tests/scratchbird_surface.mojo:226` (facade smoke asserts deterministic `connection_id` endpoint formatting)
- `tests/scratchbird_surface.mojo:554` (facade deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/scratchbird_surface.mojo:544` (facade guard SQLSTATE assertions for transport/protocol aliases and negative timeout/session DSNs, including `binarytransfer`, `protocol`, `frontdoormode`, `acquire_timeout`, and `default_row_fetch_size`)
- `tests/native_bootstrap.mojo:99` (native-bootstrap helper asserts DSN credential/host/port/database alias parsing + session/property alias parsing + protocol alias parsing + mode normalization/precedence checks)
- `tests/native_bootstrap.mojo:113` (native-bootstrap smoke asserts bracketed IPv6 host/port parsing)
- `tests/native_bootstrap.mojo:249` (native-bootstrap smoke asserts deterministic `connection_id` endpoint formatting)
- `tests/native_bootstrap.mojo:697` (native-bootstrap deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/native_bootstrap.mojo:682` (native-bootstrap guard SQLSTATE assertions for transport/protocol aliases and negative timeout/session DSNs, including `binarytransfer`, `protocol`, `frontdoormode`, `acquire_timeout`, and `default_row_fetch_size`)
