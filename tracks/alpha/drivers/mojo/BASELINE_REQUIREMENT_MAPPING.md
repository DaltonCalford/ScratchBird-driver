...
- `src/scratchbird_native.mojo:718` (native-bootstrap metadata restriction comparator supports exact/wildcard/null (`=`, `LIKE ... ESCAPE '\'`, `IS NULL`) predicates)
- `src/scratchbird_native.mojo:842` (native-bootstrap restriction-aware multi-restriction query resolution + `07001` count guard)
...
- `src/scratchbird.py:1315` (bridge-shim SQL LIKE matcher helper supports escape-aware, case-insensitive pattern evaluation)
- `src/scratchbird.py:1342` (bridge-shim deterministic schema metadata row shaping supports `=`, `LIKE`, and `IS NULL` filters)
- `src/scratchbird.py:1380` (bridge-shim metadata restriction comparator supports exact/wildcard/null (`=`, `LIKE ... ESCAPE '\'`, `IS NULL`) predicates)
- `src/scratchbird.py:1506` (bridge-shim restriction mapping normalizer + `22023` guard for non-mapping inputs)
- `src/scratchbird.py:1537` (bridge-shim restriction-aware multi-restriction query resolution)
- `src/scratchbird.py:1674` (`build_ddl_editor_schema_payload(...)` deterministic editor payload builder)
- Lane-local test anchors:
...
- `tests/metadata_execution.py:153` (bridge-shim multi-restriction query-shaping + mapping guard coverage)
- `tests/metadata_execution.py:231` (bridge-shim restricted multi-restriction execution routing coverage)
- `tests/metadata_execution.py:282` (bridge-shim restricted multi-restriction rowcount helper coverage)
- `tests/metadata_execution.py:293` (bridge-shim editor payload helper path with schema-pattern restriction shaping)
- `tests/metadata_execution.py:324` (bridge-shim `IS NULL` metadata restriction execution coverage)
- `tests/metadata_execution.py:332` (bridge-shim SQL LIKE escape-aware/case-insensitive matcher coverage)
