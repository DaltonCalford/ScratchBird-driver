...
- `src/scratchbird_native.mojo:170` (native-bootstrap protocol alias parsing: `protocol|parser|dialect`)
...
- `src/scratchbird_native.mojo:1214` (native-bootstrap connect guards: protocol/TLS/binary/compression/mode/user+db + endpoint/timeout/session/pooling/manager guardrails, including manager-proxy token requirement `08001` and non-negative guards for `default_row_fetch_size`, `min_pool_size`, `connection_lifetime`, `manager_client_flags`, plus `max_pool_size >= 1`)
- `src/scratchbird_native.mojo:1271` (native-bootstrap `connect` entrypoint)
...
- `tests/scratchbird_surface.mojo:75` (current-syntax `scratchbird` facade connect/ping/query smoke + DSN credential/host/port/database alias parse assertions (including JDBC/PG aliases), session+metadata alias parsing, pooling/manager alias parsing, protocol alias parsing, and mode normalization/precedence checks)
- `tests/scratchbird_surface.mojo:80` (facade smoke asserts bracketed IPv6 host/port parsing)
- `tests/scratchbird_surface.mojo:271` (facade smoke asserts deterministic `connection_id` endpoint formatting)
- `tests/scratchbird_surface.mojo:599` (facade deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/scratchbird_surface.mojo:593` (facade guard SQLSTATE assertions for transport/protocol/front-door aliases and negative timeout/session/pooling/manager DSNs, including manager-proxy token requirement and `default_row_fetch_size`/`min_pool_size`/`max_pool_size`/`connection_lifetime`/`manager_client_flags` guards)
- `tests/native_bootstrap.mojo:99` (native-bootstrap helper asserts DSN credential/host/port/database alias parsing + session+metadata alias parsing + pooling/manager alias parsing + protocol alias parsing + mode normalization/precedence checks)
- `tests/native_bootstrap.mojo:104` (native-bootstrap smoke asserts bracketed IPv6 host/port parsing)
- `tests/native_bootstrap.mojo:290` (native-bootstrap smoke asserts deterministic `connection_id` endpoint formatting)
- `tests/native_bootstrap.mojo:738` (native-bootstrap deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/native_bootstrap.mojo:732` (native-bootstrap guard SQLSTATE assertions for transport/protocol/front-door aliases and negative timeout/session/pooling/manager DSNs, including manager-proxy token requirement and `default_row_fetch_size`/`min_pool_size`/`max_pool_size`/`connection_lifetime`/`manager_client_flags` guards)
