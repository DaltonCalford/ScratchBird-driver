...
- `src/scratchbird_native.mojo:114` (native-bootstrap session property parsing: `role`, `application_name|applicationname`, `autocommit|auto_commit`, `readonly|read_only`, `current_schema|search_path|searchpath`, `default_row_fetch_size|fetch_size|fetchsize`)
...
- `src/scratchbird_native.mojo:219` (native-bootstrap compression normalization: `none` -> `off`)
...
- `src/scratchbird_native.mojo:1376` (native-bootstrap connect guards: malformed query-escape guard (`22023`), protocol/TLS/binary/compression/mode/user+db + endpoint/timeout/session/pooling/manager guardrails, including manager-proxy token requirement `08001`, non-negative guards for `default_row_fetch_size`, `min_pool_size`, `connection_lifetime`, `manager_client_flags`, plus `max_pool_size >= 1`, and unsupported compression guard for non-`off` values)
...
- `tests/scratchbird_surface.mojo:75` (facade helper asserts session+metadata alias parsing, prepare-threshold/batched-insert/logger alias parsing, TLS material alias/default parsing, pooling/manager alias parsing, `ssl` alias parsing, compression normalization, and URL-style query decoding)
...
- `tests/native_bootstrap.mojo:99` (native-bootstrap helper asserts session+metadata alias parsing, prepare-threshold/batched-insert/logger alias parsing, TLS material alias/default parsing, pooling/manager alias parsing, `ssl` alias parsing, compression normalization, and URL-style query decoding)
