...
- `src/scratchbird.py:1070` (bridge-shim shared metadata rowcount fallback helper returns `rowcount` when integer (excluding booleans), else `len(rows)`, else `0` with mapping/text/unsized-row fallback)
- `src/scratchbird.py:1083` (bridge-shim shared metadata rows fallback helper normalizes list/tuple rows and returns `[]` for missing/mapping/text/unsized rows)
...
- `tests/txn_exec_parity.py:606` (bridge-shim instance metadata rowcount helper fallback assertions for invalid/missing/unsized row payloads)
...
- `tests/txn_exec_parity.py:608` (bridge-shim static/instance `get_schema` fallback assertions for tuple-row normalization and mapping/text/unsized-row empty fallback)
- `tests/txn_exec_parity.py:670` (bridge-shim static/instance `ddl_editor_schema_payload` fallback assertions for tuple-row normalization and mapping/text/unsized-row empty fallback)
