...
- `src/scratchbird_native.mojo:59` (native-bootstrap front-door mode normalization, including `manager-proxy` → `manager_proxy`)
...
- `src/scratchbird_native.mojo:982` (native-bootstrap connect guards: TLS/binary/compression/mode/user+db + endpoint/timeout guardrails + deterministic auth-fail parity)
...
- `tests/scratchbird_surface.mojo:75` (current-syntax `scratchbird` facade connect/ping/query smoke + DSN host/port/timeout parse assertions + manager-proxy normalization)
...
- `tests/native_bootstrap.mojo:99` (native-bootstrap connect/ping/query smoke + DSN host/port/timeout parse assertions + manager-proxy normalization)
