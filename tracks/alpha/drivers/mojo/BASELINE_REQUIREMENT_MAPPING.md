...
- `src/scratchbird_native.mojo:76` (native-bootstrap `user|username|pguser` / `password|passwd|pgpassword` query overrides layered over DSN userinfo parsing)
- `src/scratchbird_native.mojo:88` (native-bootstrap `host|hostname|servername|pghost`, `port|portNumber|pgport`, and `database|dbname|databaseName|pgdatabase` query overrides layered over DSN endpoint/path parsing, including bracketed IPv6 hosts)
- `src/scratchbird_native.mojo:106` (native-bootstrap session property parsing: `role`, `application_name|applicationname`, `autocommit|auto_commit`, `readonly|read_only`, `current_schema|search_path|searchpath`, `default_row_fetch_size|fetch_size|fetchsize`)
- `src/scratchbird_native.mojo:116` (native-bootstrap metadata/session alias parsing for `metadata_expand_schema_parents|metadataexpandschemaparents|expand_schema_parents|expandschemaparents|dbeaver_expand_schema_parents|dbeaverexpandschemaparents`)
- `src/scratchbird_native.mojo:140` (native-bootstrap pooling parsing: `tcpkeepalive`, `pooling`, `min_pool_size|minpoolsize`, `max_pool_size|maxpoolsize`, `connection_lifetime|connectionlifetime|poolingconnectionlifetime`)
- `src/scratchbird_native.mojo:150` (native-bootstrap manager parsing: `manager_*|mcp_*` aliases and defaults for `manager_connection_profile`, `manager_client_intent`, and `manager_auth_fast_path`)
- `src/scratchbird_native.mojo:250` (native-bootstrap connection identity formatting includes endpoint context: `user@host:port/database`)
- `src/scratchbird_native.mojo:179` (native-bootstrap front-door mode normalization + source-precedence across `front_door_mode|frontdoormode` / `connection_mode` / `ingress_mode`, including `managed` normalization)
- `src/scratchbird_native.mojo:170` (native-bootstrap protocol selector parsing: `protocol|parser|dialect`)
- `src/scratchbird_native.mojo:1022` (native-bootstrap protocol canonicalization maps `scratchbird` / `scratchbird-native` / `scratchbird_native` to `native`)
- `src/scratchbird_native.mojo:192` (native-bootstrap timeout alias parsing: `connect_timeout|connecttimeout`, `socket_timeout|sockettimeout`, `login_timeout|logintimeout`, `acquire_timeout|acquiretimeout` with fallback `pooling_acquire_timeout|poolingacquiretimeout`)
- `src/scratchbird_native.mojo:188` (native-bootstrap TLS alias parsing: `sslmode|ssl`)
- `src/scratchbird_native.mojo:190` (native-bootstrap compression normalization: `none` -> `off`)
- `src/scratchbird_native.mojo:832` (native-bootstrap DSN query value decoding for `%xx` and `+`)
- `src/scratchbird_native.mojo:877` (native-bootstrap malformed query-escape detection helper)
- `src/scratchbird_native.mojo:196` (native-bootstrap lifecycle DSN knob parsing: `cb_*` / `keepalive_*` / `leak_*` / `pipeline_*`)
- `src/scratchbird_native.mojo:1322` (native-bootstrap connect guards: malformed query-escape guard (`22023`), protocol/TLS/binary/compression/mode/user+db + endpoint/timeout/session/pooling/manager guardrails, including manager-proxy token requirement `08001`, non-negative guards for `default_row_fetch_size`, `min_pool_size`, `connection_lifetime`, `manager_client_flags`, plus `max_pool_size >= 1`, and unsupported compression guard for non-`off` values)
- `src/scratchbird_native.mojo:1384` (native-bootstrap `connect` entrypoint)
- `src/scratchbird_native.mojo:420` (native-bootstrap `ping` surface)
...
- `tests/scratchbird_surface.mojo:148` (current-syntax `scratchbird` facade connect/ping/query smoke + DSN credential/host/port/database alias parse assertions (including JDBC/PG aliases), protocol alias canonicalization checks, and mode normalization/precedence checks)
- `tests/scratchbird_surface.mojo:75` (facade helper asserts session+metadata alias parsing, pooling/manager alias parsing, `ssl` alias parsing, compression normalization, and URL-style query decoding)
- `tests/scratchbird_surface.mojo:153` (facade smoke asserts bracketed IPv6 host/port parsing)
- `tests/scratchbird_surface.mojo:298` (facade smoke asserts deterministic `connection_id` endpoint formatting)
- `tests/scratchbird_surface.mojo:646` (facade deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/scratchbird_surface.mojo:607` (facade guard SQLSTATE assertions for transport/protocol/front-door/ssl/compression/malformed-query aliases and negative timeout/session/pooling/manager DSNs, including manager-proxy token requirement and `default_row_fetch_size`/`min_pool_size`/`max_pool_size`/`connection_lifetime`/`manager_client_flags` guards)
- `tests/native_bootstrap.mojo:172` (native-bootstrap helper asserts DSN credential/host/port/database alias parsing + protocol alias canonicalization + mode normalization/precedence checks)
- `tests/native_bootstrap.mojo:99` (native-bootstrap helper asserts session+metadata alias parsing, pooling/manager alias parsing, `ssl` alias parsing, compression normalization, and URL-style query decoding)
- `tests/native_bootstrap.mojo:177` (native-bootstrap smoke asserts bracketed IPv6 host/port parsing)
- `tests/native_bootstrap.mojo:317` (native-bootstrap smoke asserts deterministic `connection_id` endpoint formatting)
- `tests/native_bootstrap.mojo:780` (native-bootstrap deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/native_bootstrap.mojo:741` (native-bootstrap guard SQLSTATE assertions for transport/protocol/front-door/ssl/compression/malformed-query aliases and negative timeout/session/pooling/manager DSNs, including manager-proxy token requirement and `default_row_fetch_size`/`min_pool_size`/`max_pool_size`/`connection_lifetime`/`manager_client_flags` guards)
