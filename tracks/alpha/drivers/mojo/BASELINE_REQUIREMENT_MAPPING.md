...
- `src/scratchbird_native.mojo:55` (native-bootstrap `user|username` / `password|passwd` query overrides layered over DSN userinfo parsing)
- `src/scratchbird_native.mojo:57` (native-bootstrap `host|hostname` and `database|dbname` query overrides layered over DSN endpoint/path parsing, including bracketed IPv6 hosts)
- `src/scratchbird_native.mojo:131` (native-bootstrap connection identity formatting includes endpoint context: `user@host:port/database`)
...
- `src/scratchbird_native.mojo:75` (native-bootstrap timeout alias parsing: `connect_timeout|connecttimeout`, `socket_timeout|sockettimeout`, `login_timeout|logintimeout`, `acquire_timeout|acquiretimeout` with fallback `pooling_acquire_timeout|poolingacquiretimeout`)
- `src/scratchbird_native.mojo:72` (native-bootstrap lifecycle DSN knob parsing: `cb_*` / `keepalive_*` / `leak_*` / `pipeline_*`)
- `src/scratchbird_native.mojo:1090` (native-bootstrap connect guards: TLS/binary/compression/mode/user+db + endpoint/timeout guardrails (including `port` upper bound and non-negative `acquire_timeout`) + deterministic auth-fail parity)
- `src/scratchbird_native.mojo:1018` (native-bootstrap `connect` entrypoint)
- `src/scratchbird_native.mojo:287` (native-bootstrap `ping` surface)
...
- `tests/scratchbird_surface.mojo:75` (current-syntax `scratchbird` facade connect/ping/query smoke + DSN credential/host/port/database alias parse assertions + mode normalization/precedence checks)
- `tests/scratchbird_surface.mojo:89` (facade smoke asserts bracketed IPv6 host/port parsing)
- `tests/scratchbird_surface.mojo:144` (facade smoke asserts deterministic `connection_id` endpoint formatting)
- `tests/scratchbird_surface.mojo:426` (facade deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/scratchbird_surface.mojo:497` (facade timeout guard SQLSTATE assertions via negative timeout DSNs, including `acquire_timeout`)
- `tests/native_bootstrap.mojo:99` (native-bootstrap helper asserts DSN credential/host/port/database alias parsing + mode normalization/precedence checks)
- `tests/native_bootstrap.mojo:113` (native-bootstrap smoke asserts bracketed IPv6 host/port parsing)
- `tests/native_bootstrap.mojo:154` (native-bootstrap smoke asserts deterministic `connection_id` endpoint formatting)
...
- `tests/native_bootstrap.mojo:649` (native-bootstrap timeout guard SQLSTATE assertions via negative timeout DSNs, including `acquire_timeout`)
