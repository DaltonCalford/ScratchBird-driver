...
- `src/scratchbird_native.mojo:823` (native-bootstrap malformed bracketed-IPv6 authority detection helper)
...
- `src/scratchbird_native.mojo:1025` (native-bootstrap strict malformed-integer DSN detection helpers for primary/alias property keys)
- `src/scratchbird_native.mojo:229` (native-bootstrap lifecycle DSN knob parsing: `cb_*` / `keepalive_*` / `leak_*` / `pipeline_*`)
- `src/scratchbird_native.mojo:1434` (native-bootstrap connect guards: malformed query-escape guard (`22023`), malformed bracketed-IPv6 guard (`22023`), strict malformed-integer DSN guards (`22023`), protocol/TLS/compression/front-door/user+db + endpoint/timeout/session/pooling/manager guardrails (including invalid `front_door_mode` `0A000`, manager-proxy token requirement `08001`, non-negative guards for `default_row_fetch_size`, `min_pool_size`, `connection_lifetime`, `manager_client_flags`, plus `max_pool_size >= 1`); accepts `binary_transfer=false` and rejects unsupported compression values outside `off|zstd`)
- `src/scratchbird_native.mojo:1532` (native-bootstrap `connect` entrypoint)
...
- `tests/scratchbird_surface.mojo:697` (facade deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/scratchbird_surface.mojo:653` (facade guard SQLSTATE assertions for transport/protocol/front-door/ssl/compression/malformed-query aliases and malformed bracketed-IPv6 + malformed-integer DSN guards, plus negative timeout/session/pooling/manager DSNs, including manager-proxy token requirement and `default_row_fetch_size`/`min_pool_size`/`max_pool_size`/`connection_lifetime`/`manager_client_flags` guards)
...
- `tests/native_bootstrap.mojo:831` (native-bootstrap deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/native_bootstrap.mojo:787` (native-bootstrap guard SQLSTATE assertions for transport/protocol/front-door/ssl/compression/malformed-query aliases and malformed bracketed-IPv6 + malformed-integer DSN guards, plus negative timeout/session/pooling/manager DSNs, including manager-proxy token requirement and `default_row_fetch_size`/`min_pool_size`/`max_pool_size`/`connection_lifetime`/`manager_client_flags` guards)
