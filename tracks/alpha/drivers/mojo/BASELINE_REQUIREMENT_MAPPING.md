...
- `src/scratchbird.py:1070` (bridge-shim shared metadata rowcount fallback helper returns `rowcount` when integer, else `len(rows)`, else `0` with unsized-row fallback)
