...
- `src/scratchbird.py:1322` (bridge-shim wire begin option mapping uses normalized/coerced begin options and marks transaction active while resetting savepoints)
- `src/scratchbird.py:1359` (bridge-shim wire `commit` transitions transaction state to inactive and clears savepoints)
- `src/scratchbird.py:1370` (bridge-shim wire `rollback` transitions transaction state to inactive and clears savepoints)
- `src/scratchbird.py:1389` (bridge-shim wire savepoint helpers now enforce closed-connection guard precedence via shared static guard helper)
...
- `tests/txn_exec_parity.py:60` (begin option mapping assertions plus wire transaction activation/savepoint reset)
- `tests/txn_exec_parity.py:107` (wire begin invalid-option SQLSTATE assertions for deterministic `22023` behavior)
- `tests/txn_exec_parity.py:137` (wire commit/rollback state-transition assertions, including `_txn_id` reset and savepoint clearing)
...
- `tests/txn_exec_parity.py:442` (wire/static closed-connection guard assertions for begin/commit/rollback/savepoint/query/metadata expose SQLSTATE `08003`)
...
- `tests/txn_exec_parity.py:442` (wire/static closed-connection query + metadata helper guards expose SQLSTATE `08003`)
...
- `tests/txn_exec_parity.py:442` (bridge-shim wire/static SQLSTATE assertions for closed-connection begin/commit/rollback/savepoint/query/metadata guards `08003`)
