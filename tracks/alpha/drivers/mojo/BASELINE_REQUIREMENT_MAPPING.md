...
- `src/scratchbird_native.mojo:168` (native-bootstrap connection identity formatting includes endpoint context: `user@host:port/database`)
- `src/scratchbird_native.mojo:98` (native-bootstrap front-door mode normalization + source-precedence across `front_door_mode|frontdoormode` / `connection_mode` / `ingress_mode`, including `managed` normalization)
- `src/scratchbird_native.mojo:88` (native-bootstrap protocol alias parsing: `protocol|parser|dialect`)
- `src/scratchbird_native.mojo:110` (native-bootstrap timeout alias parsing: `connect_timeout|connecttimeout`, `socket_timeout|sockettimeout`, `login_timeout|logintimeout`, `acquire_timeout|acquiretimeout` with fallback `pooling_acquire_timeout|poolingacquiretimeout`)
- `src/scratchbird_native.mojo:114` (native-bootstrap lifecycle DSN knob parsing: `cb_*` / `keepalive_*` / `leak_*` / `pipeline_*`)
- `src/scratchbird_native.mojo:1132` (native-bootstrap connect guards: protocol/TLS/binary/compression/mode/user+db + endpoint/timeout guardrails (including `port` upper bound and non-negative `acquire_timeout`) + deterministic auth-fail parity)
...
- `tests/scratchbird_surface.mojo:75` (current-syntax `scratchbird` facade connect/ping/query smoke + DSN credential/host/port/database alias parse assertions (including JDBC/PG aliases), protocol alias parsing, and mode normalization/precedence checks)
- `tests/scratchbird_surface.mojo:89` (facade smoke asserts bracketed IPv6 host/port parsing)
- `tests/scratchbird_surface.mojo:205` (facade smoke asserts deterministic `connection_id` endpoint formatting)
- `tests/scratchbird_surface.mojo:533` (facade deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/scratchbird_surface.mojo:523` (facade guard SQLSTATE assertions for transport/protocol aliases and negative timeout DSNs, including `binarytransfer`, `protocol`, `frontdoormode`, and `acquire_timeout`)
- `tests/native_bootstrap.mojo:99` (native-bootstrap helper asserts DSN credential/host/port/database alias parsing + protocol alias parsing + mode normalization/precedence checks)
- `tests/native_bootstrap.mojo:113` (native-bootstrap smoke asserts bracketed IPv6 host/port parsing)
- `tests/native_bootstrap.mojo:228` (native-bootstrap smoke asserts deterministic `connection_id` endpoint formatting)
- `tests/native_bootstrap.mojo:676` (native-bootstrap deterministic auth-fail guard SQLSTATE assertion via `sb_test_auth_fail=true`)
- `tests/native_bootstrap.mojo:661` (native-bootstrap guard SQLSTATE assertions for transport/protocol aliases and negative timeout DSNs, including `binarytransfer`, `protocol`, `frontdoormode`, and `acquire_timeout`)
