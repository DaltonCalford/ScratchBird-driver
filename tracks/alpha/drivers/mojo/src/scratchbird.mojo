...
class ScratchBirdConnection:
    ...
    def query_metadata(self, collection_name: str = DEFAULT_METADATA_COLLECTION) -> ScratchBirdResult:
        ...
        return self.query(metadata_sql, None)

    def get_schema(self, collection_name: str = DEFAULT_METADATA_COLLECTION):
        result = self.query_metadata(collection_name)
        return result.rows
...
def normalize_metadata_collection_name(collection_name: str = DEFAULT_METADATA_COLLECTION) -> str:
    raw = str(collection_name if collection_name is not None else DEFAULT_METADATA_COLLECTION)
    normalized = raw.strip().lower().replace("-", "_").replace(" ", "_")
    if normalized == "":
        normalized = DEFAULT_METADATA_COLLECTION
    collapsed = normalized.replace("_", "")
    resolved = METADATA_COLLECTION_ALIASES.get(normalized)
    if resolved is None:
        resolved = METADATA_COLLECTION_ALIASES.get(collapsed)
    if resolved is None:
        raise ScratchBirdError("metadata collection '" + raw + "' is not supported", "0A000")
    return resolved


def resolve_metadata_collection_query(collection_name: str = DEFAULT_METADATA_COLLECTION) -> str:
    resolved = normalize_metadata_collection_name(collection_name)
    return METADATA_COLLECTION_QUERY_MAP[resolved]
