...
- Native bootstrap DSN parsing now includes transport-ready `host`/`port` fields, query override support (`host=...`, `port=...`), endpoint guards (`host` required, `port > 0`), and deterministic auth-fail guard parity via `sb_test_auth_fail=true` (`28P01`) in native/facade smoke coverage.
