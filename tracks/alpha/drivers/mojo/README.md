...
- Native bootstrap DSN parsing now includes transport-ready `host`/`port` fields, query override support (`host=...`, `port=...`), timeout aliases (`connect_timeout|connecttimeout`, `socket_timeout|sockettimeout`, `login_timeout|logintimeout`), front-door mode normalization (`manager-proxy` → `manager_proxy`), endpoint/timeout guards (`host` required, `port > 0`, timeout values `>= 0`), and deterministic auth-fail guard parity via `sb_test_auth_fail=true` (`28P01`) in native/facade smoke coverage.
