...
- Native bootstrap DSN parsing now includes transport-ready credentials/endpoint fields (`user`/`password`, `host`, `port`), query override and alias support (`user|username`, `password|passwd`, `host|hostname`, `database|dbname`, `port`), bracketed IPv6 host parsing (`[::1]:3092`), timeout aliases (`connect_timeout|connecttimeout`, `socket_timeout|sockettimeout`, `login_timeout|logintimeout`, `acquire_timeout|acquiretimeout` with fallback `pooling_acquire_timeout|poolingacquiretimeout`), front-door mode normalization (`manager-proxy`/`managerproxy` → `manager_proxy`) with key-precedence (`front_door_mode` > `connection_mode` > `ingress_mode`), endpoint/timeout guards (`host` required, `port` in `1..65535`, timeout values `>= 0`), and deterministic auth-fail guard parity via `sb_test_auth_fail=true` (`28P01`) in native/facade smoke coverage.
