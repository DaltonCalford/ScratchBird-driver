...
- native bootstrap/facade smoke now also validates DSN credential parsing (`user:password` with query overrides), endpoint parsing (`host`/`port`, including bracketed IPv6 hosts), timeout alias parsing (`connect_timeout|connecttimeout`, `socket_timeout|sockettimeout`, `login_timeout|logintimeout`), front-door mode normalization (`manager-proxy`/`managerproxy`) with source-precedence (`front_door_mode` > `connection_mode` > `ingress_mode`), timeout/endpoint guard SQLSTATE behavior (`22023` / `28000`, including out-of-range port rejection), and deterministic auth-fail guard parity (`sb_test_auth_fail=true` → `28P01`)
