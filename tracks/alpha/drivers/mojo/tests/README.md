...
- metadata execution smoke includes deterministic metadata restriction alias/query shaping helpers (including cross-collection schema/table restriction predicates) plus `query_metadata_rows(...)`/`query_metadata_rows_restricted(...)` rowcount checks in shim/native-bootstrap/facade paths
