...
- metadata execution smoke includes deterministic metadata restriction alias/query shaping helpers (including cross-collection schema/table restriction predicates, wildcard `LIKE` shaping, multi-restriction combination semantics, and invalid-restriction SQLSTATE guards) plus `query_metadata_rows(...)`/`query_metadata_rows_restricted(...)`/`query_metadata_rows_restricted_multi(...)` rowcount checks in shim/native-bootstrap/facade paths
