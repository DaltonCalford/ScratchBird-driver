...
- native bootstrap/facade smoke now also validates DSN `host`/`port` parsing, timeout alias parsing (`connect_timeout|connecttimeout`, `socket_timeout|sockettimeout`, `login_timeout|logintimeout`), front-door mode normalization (`manager-proxy`), timeout/endpoint guard SQLSTATE behavior (`22023` / `28000`), and deterministic auth-fail guard parity (`sb_test_auth_fail=true` → `28P01`)
