...
def test_begin_maps_kwargs_to_payload_flags():
    conn = TxnHarness(0)
...
def test_commit_and_rollback_noop_when_no_active_txn():
    conn = TxnHarness(0)
...
def test_commit_and_rollback_send_when_active_txn():
    conn = TxnHarness(42)
    scratchbird.ScratchBirdConnection.commit(conn)
    scratchbird.ScratchBirdConnection.rollback(conn)
...
def test_query_none_params_uses_simple_path():
    conn = QueryHarness()
...
def test_query_empty_params_uses_extended_path():
    conn = QueryHarness()
