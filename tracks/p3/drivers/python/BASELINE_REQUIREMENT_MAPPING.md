...
- `supports_dormant_reattach()` is explicit and false, and
  `detach_to_dormant()` / `reattach_dormant(...)` fail closed with `0A000`
  until the public front door exposes a real dormant token flow.
