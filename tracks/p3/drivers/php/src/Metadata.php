...
final class Metadata
{
    ...
    public const PROCEDURES_QUERY = "SELECT p.procedure_id, p.schema_id, s.schema_name, p.procedure_name, p.routine_type FROM sys.procedures p LEFT JOIN sys.schemas s ON s.schema_id = p.schema_id WHERE p.is_valid = 1 ORDER BY s.schema_name, p.procedure_name";
    public const FUNCTIONS_QUERY = "SELECT f.function_id, f.schema_id, s.schema_name, f.function_name FROM sys.functions f LEFT JOIN sys.schemas s ON s.schema_id = f.schema_id WHERE f.is_valid = 1 ORDER BY s.schema_name, f.function_name";
    public const ROUTINES_QUERY = "SELECT p.procedure_id AS routine_id, p.schema_id, s.schema_name, p.procedure_name AS routine_name, p.routine_type FROM sys.procedures p LEFT JOIN sys.schemas s ON s.schema_id = p.schema_id WHERE p.is_valid = 1 UNION ALL SELECT f.function_id AS routine_id, f.schema_id, s.schema_name, f.function_name AS routine_name, 'FUNCTION' AS routine_type FROM sys.functions f LEFT JOIN sys.schemas s ON s.schema_id = f.schema_id WHERE f.is_valid = 1 ORDER BY schema_name, routine_name";
