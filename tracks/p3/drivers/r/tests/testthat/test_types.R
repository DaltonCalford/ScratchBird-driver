...
test_that("decode primitive scalar matrix", {
  ...
  expect_equal(decode_value(SB_OID_INT8, writeBin(as.numeric(56), raw(), size = 8, endian = "little"), SB_FORMAT_BINARY), 56)
  ...
  expect_equal(decode_value(SB_OID_MONEY, writeBin(as.numeric(12345), raw(), size = 8, endian = "little"), SB_FORMAT_BINARY), 123.45)
...
test_that("decode temporal and interval matrix", {
  ...
  out_time <- decode_value(SB_OID_TIME, writeBin(as.numeric(12 * 1e6), raw(), size = 8, endian = "little"), SB_FORMAT_BINARY)
  expect_equal(as.numeric(out_time), 12)

  out_ts <- decode_value(SB_OID_TIMESTAMPTZ, writeBin(as.numeric(90 * 1e6), raw(), size = 8, endian = "little"), SB_FORMAT_BINARY)
  expect_equal(as.numeric(out_ts), as.numeric(as.POSIXct("2000-01-01 00:01:30", tz = "UTC")))

  interval_raw <- c(
    writeBin(as.numeric(5000000), raw(), size = 8, endian = "little"),
