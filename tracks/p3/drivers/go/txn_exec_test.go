...
func TestBeginTxAdoptsFreshNativeBoundaryAndRejectsNestedBegin(t *testing.T) {
	client, server := net.Pipe()
	defer client.Close()
	defer server.Close()
	...
	if !conn.hasActiveTransaction() {
		t.Fatalf("expected active transaction after adoption")
	}

	_ = server.SetReadDeadline(time.Now().Add(50 * time.Millisecond))
	_, err = readMessage(server)
	if err == nil {
		t.Fatalf("expected no wire begin message during fresh boundary adoption")
	}
...
func TestBeginTxRejectsNonDefaultFreshNativeBoundaryAdoption(t *testing.T) {
	client, server := net.Pipe()
	defer client.Close()
	defer server.Close()
