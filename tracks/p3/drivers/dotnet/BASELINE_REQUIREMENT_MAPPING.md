...
- Native `READY`, `TXN_STATUS`, and `current_txn_id` are authoritative
  transaction-state surfaces, so a fresh native session boundary can remain
  active with `txn_id == 0`.
...
|---|---|---|---|---|
| `CONN` | `JDBCBL-CONN` | `Implemented` | `src/ScratchBird.Data/ScratchBirdConnection.cs:65`, `src/ScratchBird.Data/ScratchBirdConnection.cs:146`, `src/ScratchBird.Data/ScratchBirdConnection.cs:183`, `tests/ScratchBird.Data.Tests/IntegrationTests.cs:26`, `tests/ScratchBird.Data.Tests/IntegrationTests.cs:424`, `tests/ScratchBird.Data.Tests/IntegrationTests.cs:852`, `tests/ScratchBird.Data.Tests/TransactionExecutionParityTests.cs:39` | Open/close, reconnect on stale client, and pooled reuse/recovery are implemented and integration-tested. The lane now also has deterministic proof that stale-client reconnect clears the local active-transaction claim before reopen failure can leak abandoned transaction state across sessions. |
