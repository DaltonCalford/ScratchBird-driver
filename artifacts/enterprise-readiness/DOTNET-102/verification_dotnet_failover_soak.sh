...
if [[ "${MODE}" == "runtime" ]]; then
  ...
  export SCRATCHBIRD_DOTNET_FAILOVER_SOAK_SECONDS="${SCRATCHBIRD_DOTNET_FAILOVER_SOAK_SECONDS:-300}"
  export SCRATCHBIRD_DOTNET_FAILOVER_WORKERS="${SCRATCHBIRD_DOTNET_FAILOVER_WORKERS:-6}"
...
if dotnet test "${PROJECT_PATH}" --filter FullyQualifiedName~SoakAndFaultInjectionTests.FailoverSaturationRecoveryHarness --verbosity minimal; then
