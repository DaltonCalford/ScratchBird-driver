...
function TScratchBirdIBQuery.BuildSql: string;
var
...
begin
  ...
  FResult := TScratchBirdQueryResult.Create(FDatabase.Client.ExecuteQuery(BuildSql));
  FResult.Next;
end;
...
begin
  if FDatabase = nil then
    raise Exception.Create('Database not assigned');
  FDatabase.Client.ExecSQL(BuildSql);
end;
