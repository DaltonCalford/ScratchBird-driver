...
- **Cross-lane auth-plugin architecture rollout:** Aligned Alpha-lane startup/auth surfaces to the new server plugin-auth contract by adding `client_flags|connect_client_flags`, `auth_method_payload`, `auth_required_methods`, `auth_forbidden_methods`, `auth_require_channel_binding`, `workload_identity_token`, and `proxy_principal_assertion` DSN/startup support (with namespace validation) across JDBC, .NET, Node, Go, Pascal, PHP, Ruby, Python, Rust, ODBC, and CLI/Mojo config entrypoints.
...
- **PHP lane:** Closed the 8-item parity gap batch by adding compatibility startup policy parity (`sslmode=disable`, `binary_transfer=false`, `compression=zstd|none` with invalid compression rejection), deterministic manager-proxy success/challenge/failure matrix coverage, READY-synchronized TXN error-path state handling, portal-suspend/stream continuation coverage, expanded PDO savepoint/restriction surfaces, broader metadata live assertions, and expanded per-OID type matrix coverage.
- **Pascal lane:** Closed JDBC baseline parity across CONN/TXN/EXEC/META/TYPE by enabling compatibility startup policies (`sslmode=disable`, `binary_transfer=false`, `compression=zstd|none` with invalid compression rejection), aligning direct-auth matrix assertions with startup feature negotiation, and synchronizing Pascal baseline/spec artifacts to implemented status.
- **C/C++ lane:** Closed JDBC baseline parity across CONN/TXN/EXEC/META/TYPE/ERR/RES by adding manager-proxy success/failure handshake coverage, generic plugin-auth request/continue handling, wire-level savepoint + in-flight cancel/server-statement/SBLR/attach execution coverage, DDL-editor schema payload metadata API (`schemaPattern`/`expandSchemaParents`/`schemaPaths`/`schemaTree`), parameterized batch + bulk insert resource APIs, default array/range outbound OID mapping, SQLSTATE regression coverage, JDBC-compatible DSN policy parsing (`binary_transfer=false`, `compression=zstd|off` with invalid compression rejection), and explicit IP-only transport enforcement (`inet_listener`/`managed`; no driver-side IPC/embedded).
- **Pascal lane:** Added enterprise diagnostics/observability and notification lifecycle APIs aligned to the .NET surface (`GetDiagnosticsJson`, telemetry summary/reset/slow-operation exports, Prometheus export, `Listen`/`Unlisten`/`UnlistenAll`, notification queue drain/get/clear, listener registration/removal).
- **C/C++ lane:** Added enterprise diagnostics/observability and notification lifecycle C APIs aligned to the .NET surface (`sb_get_diagnostics_json`, telemetry summary/reset/slow-operation/Prometheus exports, `sb_listen`/`sb_unlisten`/`sb_unlisten_all`/`sb_notify_channel`, notification queue drain/get/clear, listener registration/removal) plus transaction begin options (`sb_tx_begin_ex`) with wire payload parity coverage.
