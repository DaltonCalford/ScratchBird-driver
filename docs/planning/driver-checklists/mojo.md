...
- [ ] Replace Python bridge with native SBWP client in `tracks/alpha/drivers/mojo/src/scratchbird.mojo`. Progress: native bootstrap lane added in `tracks/alpha/drivers/mojo/src/scratchbird_native.mojo` with deterministic smoke coverage in `tracks/alpha/drivers/mojo/tests/native_bootstrap.mojo`; current bootstrap now includes connect/ping guards, metadata alias/query resolution, transaction lifecycle guard/no-op semantics, prepare-bind mismatch handling, prepared execute parity, paging-query rowcount semantics, and stream/cancel with post-cancel recovery smoke. Integration/conformance launchers (`tracks/alpha/drivers/mojo/tests/integration.py`, `tracks/alpha/drivers/mojo/tests/sbdriver_conformance.py`) run native-first smoke, and bridge-shim parity now includes prepare/txn/ping helpers used by the deterministic harness; full transport cutover remains open.
