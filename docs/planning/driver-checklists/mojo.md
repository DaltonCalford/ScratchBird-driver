...
- [ ] Replace Python bridge with native SBWP client in `tracks/alpha/drivers/mojo/src/scratchbird.mojo`. Progress: native bootstrap lane added in `tracks/alpha/drivers/mojo/src/scratchbird_native.mojo` with deterministic smoke coverage in `tracks/alpha/drivers/mojo/tests/native_bootstrap.mojo`; current bootstrap now includes connect/ping guards, metadata alias/query resolution, transaction lifecycle guard/no-op semantics, savepoint lifecycle guards (`25000`/`3B001`), prepare-bind mismatch handling, prepared execute parity, paging-query rowcount semantics, and stream/cancel with post-cancel recovery smoke. Integration/conformance launchers (`tracks/alpha/drivers/mojo/tests/integration.py`, `tracks/alpha/drivers/mojo/tests/sbdriver_conformance.py`) run native-first smoke, conformance now defaults to deterministic non-skipping prepare/cancel execution when `SCRATCHBIRD_MOJO_URL` is unset, bridge-shim parity includes prepare/txn/ping/savepoint helpers for deterministic harness coverage, and lifecycle scaffolds (`keepalive`/`telemetry`/`pipeline`) now compile/test under current Mojo syntax; full transport cutover remains open.
