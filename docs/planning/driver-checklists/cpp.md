...
## P1 (Core)
- [x] Expand `sb_type` and `sb_value` coverage to full SBWP type matrix in `tracks/p3/drivers/cpp/include/scratchbird/client/scratchbird_client.h`. Issue: Complete
- [x] Implement encoding/decoding for arrays, composite, range, geometry, vector, inet/cidr/macaddr in `tracks/p3/drivers/cpp/src/`. Issue: Complete
- [x] Expose SET_OPTION and PING helpers in `tracks/p3/drivers/cpp/include/scratchbird/client/scratchbird_client.h` and `tracks/p3/drivers/cpp/src/scratchbird_client_c.cpp`. Issue: Complete
- [x] Add sys.* metadata helper queries or API in `tracks/p3/drivers/cpp/include/scratchbird/client/`. Issue: Complete


## P2 (Follow-ups)
- [x] Add conformance tests for type mapping and paging in `tracks/p3/drivers/cpp/tests/`. Issue: Complete
