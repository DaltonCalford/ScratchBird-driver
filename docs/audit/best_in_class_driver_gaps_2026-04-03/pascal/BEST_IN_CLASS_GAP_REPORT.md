# Pascal/Delphi driver Best-In-Class Gap Report

Date: 2026-04-03
Lane: `pascal`
Selected benchmark: `FireDAC`
Current lane state: `full_parity`

## Verdict

ScratchBird is already at strong baseline parity but still behind the best benchmark in release/polish categories.

## Classification Counts

- `at_parity`: 10
- `partial_gap`: 4
- `full_gap`: 0
- `intentional_non_goal`: 0
- `better_than_benchmark`: 0

## Highest-Priority Gaps

- Raise packaging, IDE, and operational guidance to the standard expected by Delphi
developers.
- Add richer dataset- and component-oriented examples and validation.
- Publish performance and release evidence to support commercial-grade evaluation.

## Required Spec Closure

- Freeze FireDAC-class ergonomics and ZeosLib-class implementation anchors into the Pascal
lane supplement.

## Matrix Contract

See `BEST_IN_CLASS_GAP_MATRIX.csv` and the matching research packet under
`docs/reference/best_in_class_driver_research_2026-04-03/pascal/`.
